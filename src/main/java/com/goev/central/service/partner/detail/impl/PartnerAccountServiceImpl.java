...
public class PartnerAccountServiceImpl implements PartnerAccountService {
    ...
    @Override
    public List<PartnerAccountDto> bulkCreateAccount(String partnerUUID, List<PartnerAccountDto> partnerAccountDtoList) {
        PartnerDao partnerDao = partnerRepository.findByUUID(partnerUUID);
        if (partnerDao == null)
            throw new ResponseException("No partner found for id :" + partnerUUID);

        if (CollectionUtils.isEmpty(partnerAccountDtoList))
            throw new ResponseException("No Account present");
